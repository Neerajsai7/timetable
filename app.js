...
let appState = JSON.parse(localStorage.getItem('pathState')) || { startDate: null, progress: {}, lastLogin: null };
let chartInstance = null; let totalTasksInCurriculum = 0;
...
function getDynamicMaxDay(currentDayNumber) {
    let maxAllowedDay = currentDayNumber; let hoursLeftToday = 24 - new Date().getHours(); let allDoneSoFar = true;
...
function renderApp() {
    ...
    learningPath.forEach((dayData) => {
        ...
        if (dayData.day > maxAllowedDay) { status = 'locked'; statusText = '(Locked)'; isLocked = true; badgeColor = 'var(--text-muted)'; } 
        else if (dayData.day > currentDayNumber && dayData.day <= maxAllowedDay) { status = 'today'; statusText = '⚡ OVERDRIVE'; badgeColor = 'var(--warning)'; card.style.borderColor = 'var(--warning)'; card.style.boxShadow = '0 0 15px rgba(245, 158, 11, 0.2)'; if (dayData.day === maxAllowedDay) card.classList.add('active'); } 
        else if (dayData.day === currentDayNumber) { status = 'today'; statusText = '(Today)'; badgeColor = 'var(--success)'; card.classList.add('active'); } 
        else {
            let allDone = true;
        ...
        dayData.subtopics.forEach((sub, i) => {
            ...
            let taskDisplay = '';
            if (taskData) { completedTasksCount++; const earnedXP = taskData.status === 'on-time' ? XP_BASE : XP_LATE; totalXP += earnedXP; taskDisplay = `<span class="xp-tag">+${earnedXP} XP</span>`; } 
            else if (status === 'overdue') { const penalty = (currentDayNumber - dayData.day) * PENALTY_PER_DAY; totalXP -= penalty; taskDisplay = `<span class="penalty-tag">-${penalty} XP</span>`; } 
            else if (isLocked) { taskDisplay = `<span class="xp-tag" style="color: var(--text-muted); background: transparent;">🔒</span>`; } 
            else if (statusText === '⚡ OVERDRIVE') { taskDisplay = `<span class="xp-tag" style="color: var(--warning); border: 1px solid var(--warning);">+${XP_BASE} XP</span>`; } 
            else taskDisplay = `<span class="xp-tag">+${XP_BASE} XP possible</span>`;

            // Inline Link Logic
            let linkHTML = '';
            if (sub.url && !isLocked) { linkHTML = `<a href="${sub.url}" target="_blank" class="inline-link">📖 Study</a>`; }
...
function updateGamificationUI(xp, streak) {
    let currentLevel = xp > 0 ? Math.floor(xp / 500) + 1 : 1; let titles = ["Novice", "Apprentice", "Coder", "Architect", "Master", "Grandmaster", "Legend"];
    document.getElementById('xp-display').innerText = xp; document.getElementById('level-display').innerText = currentLevel; document.getElementById('level-title').innerText = titles[Math.min(currentLevel - 1, titles.length - 1)]; document.getElementById('streak-display').innerText = `${streak} 🔥`;
}
function updateProgressUI(completed) {
    ...
    document.getElementById('percentage-display').innerText = `${percentage}%`; document.getElementById('tasks-completed-display').innerText = `${completed} / ${totalTasksInCurriculum} Tasks Completed`; document.getElementById('main-progress-bar').style.width = `${percentage}%`;
}
function renderChart() {
    const ctx = document.getElementById('progressChart').getContext('2d');
    chartInstance = new Chart(ctx, { type: 'line', data: { labels: learningPath.map(d => `Day ${d.day}`), datasets: [{ label: 'Tasks Completed', data: calculateCumulativeProgress(), borderColor: '#3b82f6', backgroundColor: 'rgba(59, 130, 246, 0.2)', borderWidth: 2, fill: true, tension: 0.3, pointBackgroundColor: '#10b981', pointRadius: 0 }] }, options: { responsive: true, maintainAspectRatio: false, scales: { y: { beginAtZero: true, suggestedMax: totalTasksInCurriculum, grid: { color: '#334155' }, ticks: { color: '#94a3b8' } }, x: { grid: { color: '#334155' }, ticks: { display: false } } }, plugins: { legend: { labels: { color: '#f8fafc' } } } } });
}
